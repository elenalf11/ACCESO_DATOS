...
public class ShoppingCart {
	...
	public void mostrarCarrito() {
		...
		for (String product : this.cart.keySet()) {
			System.out.println(
					product + "- Cantidad: " + this.cart.get(product) + " - Precio: " + calcularTotal(product) + "€");
			total += calcularTotal(product);
		}
	...
	public double calcularTotal(String product) {
		double total = this.cart.get(product) * this.prices.get(product);
		if (total > 20) {
